...
public class Charts extends BaseTest {
  ...
  // Log instance
  private final Logger log = LogManager.getLogger( Charts.class );
   ...
   * ############################### Test Case 0 ###############################
   *
   * Description: Opens Charts tutorial and check if the page was loaded.
   */
  @Test
  public void tc0_openCharts() {
    this.log.info( "openCharts" );

    // Open CDE Tutorials Welcome page
    this.elemHelper.Get( driver, PageUrl.CDF_TUTORIAL_CHARTS );

    // Test if page was loaded
    assertEquals( this.elemHelper.WaitForElementPresentGetText( driver, By.xpath( "//*[@id='title']/h1" ) ), "Charts" );
   ...
   * The function will call another function that will check if every tab has a 
   * right panel with a title similar to the tab name. 
   * 
   * Test Steps: 
  ...
  public void tc3_checkTooltips() {
    this.cdftutorials.checkTooltips( 6 );
  }
}
